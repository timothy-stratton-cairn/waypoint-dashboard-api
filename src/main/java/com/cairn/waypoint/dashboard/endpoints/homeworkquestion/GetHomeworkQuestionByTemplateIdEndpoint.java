...
public class GetHomeworkQuestionByTemplateIdEndpoint {
  ...
  public GetHomeworkQuestionByTemplateIdEndpoint(
  ...
  @Operation(
      ...
      responses = {
          ...
          @ApiResponse(responseCode = "403", description = "Forbidden",
              content = {@Content(schema = @Schema(hidden = true))})})
  public ResponseEntity<?> getAllQuestionsByTemplateId(@PathVariable Long templateId,
      Principal principal) {
    log.info("User [{}] is retrieving all homework of the template [{}]", principal.getName(),
        templateId);

    List<HomeworkQuestion> questions = homeworkQuestionDataService.getHomeworkQuestionsByTemplateId(
        templateId);
