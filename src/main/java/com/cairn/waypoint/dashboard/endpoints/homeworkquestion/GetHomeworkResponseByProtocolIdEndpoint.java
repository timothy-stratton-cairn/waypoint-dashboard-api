...
public class GetHomeworkResponseByProtocolIdEndpoint {
  ...
  public GetHomeworkResponseByProtocolIdEndpoint(
      HomeworkQuestionResponseHelperService homeworkResponseHelperService,
  ...
  @Operation(
      ...
      responses = {
          ...
          @ApiResponse(responseCode = "403", description = "Forbidden",
              content = {@Content(schema = @Schema(hidden = true))})})
  public ResponseEntity<?> getAllResponseByProtocolId(@PathVariable Long protocolId,
      Principal principal) {
    log.info("User [{}] is retrieving all homework of the template [{}]", principal.getName(),
        protocolId);

    HomeworkResponseListDto response = homeworkResponseDataService.getHomeResponseByProtocol_Id(
        protocolId);
